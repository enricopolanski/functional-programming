...
export const chunksOf = <A>(
  ...
  return isOutOfBound(n - 1, as)
    ? [as]
    : chop(as, as => split(as, n))
}
...
export const batchOf = <A>(
  ...
  return chunksOf(tasks, n).reduce(
    (b: Task<Array<A>>, chunk: Array<Task<A>>) =>
      b.chain(xs =>
        sequenceTasks(chunk).map(ys => xs.concat(ys))
      ),
    task.of([])
  )
}
...
const withLog = <A>(
  ...
  return fromIO(log(`[START] ${message}`))
    .chain(() => fa)
    .chain(a =>
      fromIO(log(`[END] ${message}`)).map(() => a)
    )
}

const mkTestTask = <A>(a: A, d: number = 1000): Task<A> =>
  withLog(String(a), delay(d, a))
