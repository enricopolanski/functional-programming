...
半群に入る前に、まずは簡単な代数的構造の例、 _亜群（またはマグマ、magma）_ を見てみましょう。
...
- 集合`A`
- 集合`A`の上の二項演算`concat`。この演算は集合`A`において _閉じている（closed）_ と言います。つまり、`A`の任意の元`a`、`b`に対して、この演算の結果は再び`A`に属する。言い換えれば、`concat`は`A`の`コンビネータ`です.
...
**定義**：集合`A`と`A`において閉じている二項演算`*`の対`(A, *)`を _亜群(またはマグマ、magma)_ と呼ばれている。
...
- **结合律**: `A`の各元`x`，`y`，`z`に対して、以下の式が成り立つ。
...
`ReadonlyArray<string>`の半群を実装してみましょう。
...
一つの半群のインスタンスに対して、演算量の順番を入れ替えるだけで、新しい半群のインスタンスを取得できます。
...
`A`の自由半群は、データコンテンツを保持しながら、`A`の要素を連結する遅延的な方法とみなすことができます。
...
2. `Semigroup<User>`が定義てきない，あるいはマージの実現を使用者に任せたい。
...
おそらく高価で無意味な計算の実行を回避します
半群インスタンスの回り込みを回避します
これにより、API コンシューマは、(concatAll を使用して) どれが正しいマージ戦略であるかを決定できるようになります。
...
_順序_ について話す前に、まず、_等しい_ の概念について検討する必要があります。
