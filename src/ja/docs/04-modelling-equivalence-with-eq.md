...
`Set`が`===`(「厳密等価」)を使って値の比較を行うことを考慮すると、`points`には`{ x: 0, y: 0 }`が2つ含まれることになります。これは望ましくない結果です。したがって、要素を`Set`に追加するには、`Eq`抽象化を利用する新しいAPIを定義すると便利です。

**クイズ**：このAPIシグネチャーはなんでしょうか？

`EqPoint`にはテンプレートコードが多い？いいお知らせは、理論が再び`Point`のような構造体の`Eq`インスタンスの実現可能性を提供してくれました。フィールドごとに`Eq`インスタンスを実現すればいいです。
