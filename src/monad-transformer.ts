...
export const swapOption = <M>(M: Monad<M>) => <A>(
  nma: Option<HKT<M, A>>
): HKT<M, Option<A>> => {
...
export const swapEither = <M>(M: Monad<M>) => <L, A>(
...
export const swapReader = <M>(M: Monad<M>) => <E, A>(
...
export const swapState = <M>(M: Monad<M>) => <S, A>(
  nma: HKT<M, (s: S) => [A, S]>
): ((s: S) => HKT<M, [A, S]>) => {
