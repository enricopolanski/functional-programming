...
\begin{observation}
L'ultima legge permette delle ottimizzazioni
...
\begin{demo}
Non tutte le istanze di funtore sono associabili ad un "contenitore"
\end{demo}
...
\subsection{Semigruppi per higher kinds}
...
Come funziona \texttt{getOptionSemigroup}
...
\begin{verbatim}
/**
 * Changes the value of the local context
 * during the execution of the action `fa`
 */
...
\begin{verbatim}
/**
 * Modifies the result to include the
 * changes to the accumulator
 */
...
\begin{verbatim}
/**
 * Projects a value from modifications made
 * to the accumulator during an action
 */
...
export const bracket = <L, A, R>(
  ...
  return acquire.chain(r =>
    attempt(program(r)).chain(result => release(r).chain(() => result.fold<TaskEither<L, A>>(fromLeft, taskEither.of)))
  )
}
\end{verbatim}
