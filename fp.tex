...
E se sì, il monoide generato che caratteristiche ha?
...
E in particolare non sappiamo ancora come comporre programmi con effetti, guardate cosa può accadere con \texttt{Option}
...
Chiamiamo \emph{Kleisli arrow} i morfismi come $g$ ed $f$, ovvero i morfismi il cui target è l'immagine di $M$.
...
\begin{itemize}
  ...
  \item mentre la computazione si sta svolgendo, possiamo notificargli che non siamo più interessati al risultato e la computazione può scegliere di terminarsi da sola
...
\footnote{$\{0\}$ per \texttt{Tuple1}, $\{0, 1\}$ per \texttt{Tuple2}, $\{0, 1, 2\}$ per \texttt{Tuple3}}
...
Data una istanza di \texttt{Applicative} per \texttt{F} è possibile definire la seguente funzione
