...
Infine abbiamo già visto un paio di implementazioni (per \texttt{Option} e \texttt{Either})
in cui l'argomento \texttt{fa} è implicito (\texttt{this})
...
\xymatrix{
  ...
  G(A) \ar[u]^G \ar[r]^{G(g)} \ar[r] & G(B) \ar[u]^G \ar[r]^{G(f)} & G(C) \ar[u]^G & \mathcal{D} \\
