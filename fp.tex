...
Un esempio standard di tipo che ammette una istanza di funtore controvariante è
...
\begin{itemize}
  \item i predicati \texttt{(a: A) => boolean}
  \item le relazioni di equivalenza \texttt{(x: A) => (y: A) => boolean}
  \item le funzioni di ordinamento \texttt{(x: A) => (y: A) => number}
  \item i serializzatori, ad esempio \texttt{(a: A) => string}
\end{itemize}
